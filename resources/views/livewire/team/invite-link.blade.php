@can('manageInvitations', currentTeam())
    <form wire:submit='viaLink' class="flex gap-2 flex-col lg:flex-row items-end">
        ...
        <div class="flex gap-2 lg:w-fit w-full">
            <x-forms.button type="submit">{{ __('Generate Invitation Link') }}</x-forms.button>
            @if (is_transactional_emails_enabled())
                <x-forms.button wire:click.prevent='viaEmail'>{{ __('Send Invitation via Email') }}</x-forms.button>
            @endif
        </div>
    </form>
@endcan
