        ...
        @if ($modal_mode)
            ...
            @if (!isset($provider) || empty($provider) || $provider === '')
                <x-forms.select required id="provider" label="Provider">
                    <option value="hetzner">{{ __('Hetzner') }}</option>
            ...
            @if (auth()->user()->currentTeam()->cloudProviderTokens->where('provider', $provider)->isEmpty())
                <div class="text-sm text-neutral-500 dark:text-neutral-400">
                    {{ __('Create an API token in the') }} <a
                        href='{{ $provider === 'hetzner' ? 'https://console.hetzner.com/projects' : '#' }}'
                        target='_blank' class='underline dark:text-white'>{{ ucfirst($provider) }} {{ __('Console') }}</a> {{ __('→ choose Project → Security → API Tokens.') }}
                    @if ($provider === 'hetzner')
                        <br><br>
                        {{ __("Don't have a Hetzner account?") }} <a href='https://coolify.io/hetzner' target='_blank'
                            class='underline dark:text-white'>{{ __('Sign up here') }}</a>
                        <br>
                        <span class="text-xs">{{ __("(Coolify's affiliate link, only new accounts - supports us (€10) and gives you €20)") }}</span>
                    @endif
                </div>
            @endif
            ...
            <div class="flex gap-2 items-end flex-wrap">
                <div class="w-64">
                    <x-forms.select required id="provider" label="Provider" disabled>
                        <option value="hetzner" selected>{{ __('Hetzner') }}</option>
                        <option value="digitalocean">{{ __('DigitalOcean') }}</option>
                    </x-forms.select>
                </div>
            ...
            <div class="flex-1 min-w-64">
                ...
                @if (auth()->user()->currentTeam()->cloudProviderTokens->where('provider', $provider)->isEmpty())
                    <div class="text-sm text-neutral-500 dark:text-neutral-400 mt-2">
                        {{ __('Create an API token in the') }} <a href='https://console.hetzner.com/projects' target='_blank'
                            class='underline dark:text-white'>{{ __('Hetzner Console') }}</a> {{ __('→ choose Project → Security → API Tokens.') }}
                        <br><br>
                        {{ __("Don't have a Hetzner account?") }} <a href='https://coolify.io/hetzner' target='_blank'
                            class='underline dark:text-white'>{{ __('Sign up here') }}</a>
                        <br>
