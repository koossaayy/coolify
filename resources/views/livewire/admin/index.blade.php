<div>
    ...
    <div class="flex gap-2 pt-4">
        <h3>{{ __('Who am I now?') }}</h3>
        @if (session('impersonating'))
            <x-forms.button wire:click="back">{{ __('Go back to root') }}</x-forms.button>
        @endif
    </div>
    ...
    <div class="pt-4">{{ __('Active Subscribers :') }} {{ $activeSubscribers }}</div>
    <div>{{ __('Inactive Subscribers :') }} {{ $inactiveSubscribers }}</div>
    @if ($search)
                ...
                @foreach ($foundUsers as $user)
                    <div class="box w-64 group" wire:click="switchUser({{ $user->id }})">
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="box-description">{{ __('Active:') }}
        ...
        @else
            <div>{{ __('No users found with') }} {{ $search }}</div>
        @endif
    @endif
</div>
