                        ...
                        <div class="py-2">
                            ...
                            @if ($server->isFunctional())
                                @if (isset($serverContainers[$server->id]) && count($serverContainers[$server->id]) > 0)
                                    ...
                                    @endforeach
                                @else
                                    <div class="pt-2">{{ __('No containers are running on server:') }} {{ $server->name }}</div>
                                @endif
                            @else
                                <div class="pt-2">{{ __('Server') }} {{ $server->name }} {{ __('is not functional.') }}</div>
                            @endif
                        </div>
                    @empty
