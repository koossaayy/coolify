<div>
    ...
    <div class="flex flex-col h-full gap-8 sm:flex-row">
        <div class="flex flex-col items-start gap-2 min-w-fit">
            <a class="menu-item sm:min-w-fit" target="_blank" href="{{ $service->documentation() }}">{{ __('Documentation') }}
                <x-external-link /></a>
            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.service.configuration', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}">{{ __('General') }}</a>
            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.service.environment-variables', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}">{{ __('Environment Variables') }}</a>
            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.service.storages', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}">{{ __('Persistent Storages') }}</a>
            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.service.scheduled-tasks.show', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}">{{ __('Scheduled Tasks') }}</a>
            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.service.webhooks', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}">{{ __('Webhooks') }}</a>
            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.service.resource-operations', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}">{{ __('Resource Operations') }}</a>

            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.service.tags', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}">{{ __('Tags') }}</a>

            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.service.danger', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}">{{ __('Danger Zone') }}</a>
        </div>
        <div class="w-full">
            ...
            @elseif ($currentRoute === 'project.service.storages')
                ...
                <div class="pb-4 dark:text-warning text-coollabs">{{ __('If you would like to add a volume, you must add it to your compose file (') }}<a class="underline"
                        href="{{ route('project.service.configuration', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}">{{ __('General tab') }}</a>).</div>
                @foreach ($applications as $application)
