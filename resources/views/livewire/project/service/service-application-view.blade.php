<div>
    <form wire:submit='submit'>
        ...
        <div class="flex flex-col gap-2">
            @if($requiredPort && !$application->serviceType()?->contains(str($application->image)->before(':')))
                <x-callout type="warning" title="Required Port: {{ $requiredPort }}" class="mb-2">
                    {{ __('This service requires port') }} <strong>{{ $requiredPort }}</strong> {{ __("to function correctly. All domains must include this port number (or any other port if you know what you're doing).") }}
                    <br><br>
                    <strong>{{ __('Example:') }}</strong> http://app.coolify.io:{{ $requiredPort }}
                </x-callout>
            @endif
        ...
        <h3 class="py-2 pt-4">{{ __('Advanced') }}</h3>
                    ...
                    <div x-show="modalOpen" x-trap.inert.noscroll="modalOpen" x-transition:enter="ease-out duration-100"
                        ...
                        <div class="relative w-auto">
                            <x-callout type="warning" title="Port Requirement Warning" class="mb-4">
                                {{ __('This service requires port') }} <strong>{{ $requiredPort }}</strong> {{ __('to function correctly. One or more of your domains are missing a port number.') }}
                            </x-callout>
