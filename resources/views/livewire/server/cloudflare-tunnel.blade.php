            ...
            <div class="flex flex-col gap-2 pt-6">
                ...
                @elseif (!$server->isFunctional())
                    <x-callout type="info" title="Configuration Options" class="mb-4">
                        ...
                        <br />
                        {{ __('To') }} <span class="font-semibold">{{ __('manually') }}</span> {{ __('configure Cloudflare Tunnel, please click') }} <span wire:click="manualCloudflareConfig" class="underline cursor-pointer">{{ __('here') }}</span>{{ __(', then you should validate the server.') }}
                        <br /><br />
                ...
                @if (!$isCloudflareTunnelsEnabled && $server->isFunctional())
                    <div class="flex  flex-col pb-2">
                        ...
                        <a href="https://coolify.io/docs/knowledge-base/cloudflare/tunnels/server-ssh" target="_blank"
                            class="text-xs underline hover:text-yellow-600 dark:hover:text-yellow-200">{{ __('Docs') }}<x-external-link /></a>
                    </div>
