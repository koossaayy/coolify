<div>
    @if (subscriptionProvider() === 'stripe')
        <div class="pt-4">
            <h2>{{ __('Your current plan') }}</h2>
            <div class="pb-4">{{ __('Tier:') }} <strong class="dark:text-warning">
            ...
            @if (currentTeam()->subscription->stripe_cancel_at_period_end)
                <div class="pb-2">{{ __('Subscription is active but on cancel period.') }}</div>
            @else
                <div class="pb-2">{{ __('Subscription is active. Last invoice is') }}
                    {{ currentTeam()->subscription->stripe_invoice_paid ? 'paid' : 'not paid' }}.</div>
            @endif
            <div class="flex items-center gap-2">
                <div class="w-48">{{ __('Number of paid servers:') }}</div>
                <div class="text-xl font-bold dark:text-white">{{ $server_limits }}</div>
            </div>
            <div class="flex items-center gap-2">
                <div class="w-48">{{ __('Currently active servers:') }}</div>
            ...
            @if (currentTeam()->serverOverflow())
                <x-callout type="danger" title="WARNING" class="my-4">
                    {{ __('You must delete') }} {{ currentTeam()->servers->count() - $server_limits }} {{ __('servers, or upgrade your subscription.') }} {{ currentTeam()->servers->count() - $server_limits }} {{ __('servers will be deactivated.') }}
                </x-callout>
            @endif
            <x-forms.button class="gap-2" wire:click='stripeCustomerPortal'>{{ __('Change Server Quantity') }}
            </x-forms.button>
            <h2 class="pt-4">{{ __('Manage your subscription') }}</h2>
            <div class="pb-4">{{ __('Cancel, upgrade or downgrade your subscription.') }}</div>
            <div class="flex gap-2">
                <x-forms.button class="gap-2" wire:click='stripeCustomerPortal'>{{ __('Go to') }} <svg
        ...
        <div class="pt-4">
            {{ __('If you have any problems, please') }} <a class="underline dark:text-white" href="{{ config('constants.urls.contact') }}"
